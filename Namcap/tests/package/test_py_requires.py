...
"""
	...
	def test_missing_dependency(self):
		...
		self.assertEqual(dep_errors, [(
			...
			('python-six', "package python-six needed in Python packages {'foo'}"))
		])
	...
	def test_missing_distribution(self):
		...
		self.assertEqual(rule_errors, [
			('python-package-not-found %s', 'does_not_exist'),
		])
		self.assertEqual(dep_errors, [])
	...
	def test_version_mismatch(self):
		...
		self.assertEqual(
			(tag, needed_version),
			('python-package-version-mismatch %s %s', 'six>=999999'),
		)
		self.assertEqual(dep_errors, [])

	# TODO: more tests - mismatched python version, complex markers, multiple dists in an ALPM package
